...
public abstract class HttpRunner implements OnEventRunner {
	...
	protected String addUrlCommonParams(String url){
		...
		.append("&deviceuuid=")
		.append(strDeviceUUID + "-" + Encrypter.encryptBySHA1(strDeviceUUID + KEY_HTTP));
		if(mAddUserParam && !TextUtils.isEmpty(strUser)){
			buf.append("&user=")
			.append(URLEncoder.encode(strUser + "-" + Encrypter.encryptBySHA1(strUser + KEY_HTTP)));
		}
		buf.append("&timesign=")
		.append(lTime + "-" + Encrypter.encryptBySHA1(lTime + KEY_HTTP));
